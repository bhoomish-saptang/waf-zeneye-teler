...
import (
	...
	"github.com/waf-teler-zeneye/database"
	"go.uber.org/zap"
...
func Update(opts ...UpdateOptions) *Teler {
	...
	var RuleBody database.CustomRules
	...
	var checkvar bool
	RuleMap, checkvar := database.RetrieveRule(RuleBody.Rules, checkvar)
	if len(RuleMap) == 0 {
		fmt.Println("error Retrieving")
	}
	if !checkvar {
		fmt.Println("Retrieving the Rule error from DB")
	}

	Rule.Rulename = RuleMap["rulename"]
	Rule.Action = RuleMap["action"]
	Rule.Field = RuleMap["field"]
	Rule.FieldVal = RuleMap["value"]
	Rule.Condition = RuleMap["condition"]
